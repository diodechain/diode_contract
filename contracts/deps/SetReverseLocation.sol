...
// Same as Set, but items slot is first to maintain slot positions
// when replacing an array with a set
library SetReverseLocation {
    struct Data {
        address[] items;
    ...
    function Clear(Data storage self) internal {
        for (uint256 i = self.items.length; i > 0; i--) {
