...
contract DiodeStake {
    ...
    function _contractStake(IFleetContract _fleet) internal view returns (Stake.Data memory) {
        return contractStake[address(_fleet)];
    }

    event Staked(bool indexed isContract, address indexed target, uint256 indexed amount);

    event Unstaked(bool indexed isContract, address indexed target, uint256 indexed amount);

    event Withdrawn(bool indexed isContract, address indexed target, uint256 indexed amount);
