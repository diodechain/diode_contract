...
contract DiodeRegistry is DiodeStake {
    ...
    function _fleetStats(IFleetContract _fleet) internal view returns (FleetStats storage) {
    ...
    event Ticket(IFleetContract indexed fleetContract, address indexed node, address indexed client);
    ...
    function EpochFleetNode(IFleetContract _fleet, address _node) external view returns (Node memory) {
        ...
        uint256 len = nstats.clientArray.length;
