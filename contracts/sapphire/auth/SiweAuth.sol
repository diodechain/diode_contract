...
import {Strings} from "./../../deps/Strings.sol";
...
contract SiweAuth is A13e {
    ...
    function siwe_initialize(string memory inDomain) public {
        require(bytes(_domain).length == 0, "Already initialized");
        _authTokenEncKey = bytes32(Sapphire.randomBytes(32, ""));
        _domain = inDomain;
    }
    ...
    function login(string calldata siweMsg, SignatureRSV calldata sig) external view override returns (bytes memory) {
        AuthToken memory b = login_test(siweMsg, sig);
        bytes memory encB = Sapphire.encrypt(_authTokenEncKey, 0, abi.encode(b), "");
        return encB;
    }

    function login_test(string calldata siweMsg, SignatureRSV calldata sig) public view returns (AuthToken memory) {
        AuthToken memory b;
        ...
        if (p.notBefore.length != 0 && block.timestamp <= SiweParser.timestampFromIso(p.notBefore)) {
            revert SiweAuth_NotBeforeInFuture();
        }

        if (p.expirationTime.length != 0) {
            // Compute expected block number at expiration time.
            uint256 expirationTime = SiweParser.timestampFromIso(p.expirationTime);
    ...
    function decodeAndValidateToken(bytes memory token) internal view virtual returns (AuthToken memory) {
        bytes memory authTokenEncoded = Sapphire.decrypt(_authTokenEncKey, 0, token, "");
