...
contract BNSTest is Test {
    ...
    function testGoodname() public {
        ...
        Assert.equal(
            ...
            msg.sender,
            "name should resolve to msg.sender"
        );
    }
    ...
    function testInvalidNamesRevert() public {
        ...
        fail(
            ...
            "registering a too long name should revert"
        );
        fail(
            "-loooooooong",
            "registering a name beginning with - should revert"
        );
    ...
    function testDoubleRegister() public {
        ...
        Assert.equal(
            ...
            true,
            "registering again from same "
        );
        Assert.equal(
            instance.Resolve(name),
            address(this),
            "name should resolve to this"
        );
    }
    ...
    function testReverse() public {
        ...
        Assert.equal(
            ...
            "ResolveReverse(address(msg.sender)) should be empty"
        );
        Assert.equal(
            empty,
            instance.ResolveReverse(address(this)),
            "ResolveReverse(address(this)) should be empty"
        );

        // Both should RegisterReverse fine, because both are in the forward entry:
        Assert.ok(
            _registerReverse(address(msg.sender), name),
            "registering address(msg.sender) should work"
        );
        Assert.ok(
            _registerReverse(address(this), name),
            "registering address(this) should work"
        );
        // Registering another a not forward mapped address should fail
        Assert.notOk(
            _registerReverse(address(0), name),
            "registering not mapped address should fail"
        );
        ...
        Assert.equal(
            name,
            instance.ResolveReverse(address(this)),
            "ResolveReverse(address(this)) should match address"
        );
        ...
        Assert.notOk(
            _registerReverse2(address(msg.sender), name2),
            "overriding msg.sender assignment should succeed"
        );
        Assert.notOk(
            _registerReverse2(address(this), name2),
            "overriding this assignment should fail"
        );

        Assert.ok(
            _unregisterReverse(address(msg.sender)),
            "clearing msg.sender assignment should succeed"
        );
        Assert.ok(
            _unregisterReverse(address(this)),
            "clearing this assignment should succeed"
        );
    }
    ...
    function testProperties() public {
        ...
        Assert.equal(
            prop0,
            instance.GetProperty(name, 0),
            "should match set-0 value"
        );
        ...
        Assert.equal(
            prop1,
            instance.GetProperty(name, 1),
            "should match set-1 value"
        );
        ...
        // Deleting a prop
        Assert.equal(
            instance.GetPropertyLength(name),
            2,
            "we should have two props"
        );
        instance.DeleteProperty(name, 0);
        Assert.equal(
            "author=Dominic Letz",
            instance.GetProperty(name, 0),
            "should match set-0 value after delete"
        );
        Assert.equal(
            instance.GetPropertyLength(name),
            1,
            "we should have one prop left"
        );
    }
    ...
    function _registerReverse(
        address target,
        string memory name
    ) internal returns (bool) {
        (bool success, ) = address(instance).call{gas: gasleft()}(
            abi.encodeWithSignature(
                "RegisterReverse(address,string)",
                target,
                name
            )
        );
    ...
    function _unregisterReverse(address target) internal returns (bool) {
        (bool success, ) = address(instance).call{gas: gasleft()}(
            abi.encodeWithSignature("UnregisterReverse(address)", target)
        );
        return success;
    }

    function _registerReverse2(
        address target,
        string memory name
    ) internal returns (bool) {
        (bool success, ) = address(instanceVia).call{gas: gasleft()}(
