...
export FLEET_CONTRACT_FACTORY=0xa21D0a54dFee3Ff4B9f82959C09B538863744839
...
set -e
OLD_FLEET_CONTRACT_IMPLEMENTATION=`cast call $FLEET_CONTRACT_FACTORY "GetFleetContractImplementation()" --rpc-url $RPC | cast parse-bytes32-address`
OLD_VSN=`cast call $OLD_FLEET_CONTRACT_IMPLEMENTATION "Version()" --rpc-url $RPC | cast td`
if [ "$OLD_VSN" == "" ]; then
...
OLD_HASH=$(cast keccak $(cast code $OLD_FLEET_CONTRACT_IMPLEMENTATION --rpc-url $RPC))
...
set -x
cast send --legacy --rpc-url $RPC --private-key $(cat diode_glmr.key) $FLEET_CONTRACT_FACTORY "SetFleetContractImplementation(address)" $1
