...
export FOUNDRY_REMAPPINGS_DEV=cross=`pwd`/lib/moonbeam
# Cancun supported since RT3000 July 2024 https://forum.moonbeam.network/t/runtime-rt3000-schedule/1752/2
forge create --broadcast --evm-version cancun --optimize --optimizer-runs 200 --rpc-url $RPC --private-key $(cat diode_glmr.key) --verify --verifier-url https://api-moonbeam.moonscan.io/api -e $(cat moonscan_api.key) $*
