...
TYPE=`cast call $1 "Type()" --rpc-url $RPC`
if [ "$TYPE" == "" ]; then
    echo "Failed to get type"
    exit 1
fi

OLD_IMPL_ADDR=`cast call $PROXY "resolve(bytes32)" $TYPE --rpc-url $RPC | cast parse-bytes32-address`

echo "Old implementation address $OLD_IMPL_ADDR"
OLD_VSN=`cast call $OLD_IMPL_ADDR "Version()" --rpc-url $RPC | cast td`
if [ "$OLD_VSN" == "" ]; then
...
OLD_HASH=$(cast keccak $(cast code $OLD_IMPL_ADDR --rpc-url $RPC))
